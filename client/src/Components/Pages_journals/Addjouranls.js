...
const Addjouranls = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('http://localhost:8080/articles', values)
      .then((response) => {
  ...
  return (
    <>
      <div className='bar-add gap-4'>
        <Link to="/issues/journals/entries/status/1/2020/" className="w-auto p-2 text-center bg-gray-400 rounded text-white uppercase text-sm">
        ...
        <button onClick={handleSubmit} className=" w-auto p-2 text-center bg-blue-700 rounded text-white uppercase text-sm">
          Submit
        </button>
      </div>
      <div>
        <div className="grid grid-cols-4 gap-4 p-4 mt-14">
          ...
          <div>
            <label className='uppercase font-medium text-sm'>Phone</label>
            <input
              className={`w-full border-2 border-gray-300 p-2 focus:outline-emerald-600 rounded-lg`}
          ...
          <div>
            <label className='uppercase font-medium text-sm'>Raw File<span className='text-red-600'>*</span> </label>
          ...
          {/* <button className=" w-auto p-2 text-center bg-blue-700 rounded text-white uppercase text-sm">
            Submit
          </button> */}
        </div>
      </div >
